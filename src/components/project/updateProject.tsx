import { useProjectStore, useStateStore } from "@/Zustand/store";
import { Button, Form, Input, List, Spin, message } from "antd";
...
const UpdateProjectModal = ({ project }: { project: any }) => {
  const [isRenderd, setRenderd] = useState(false);
  useEffect(() => {
    setRenderd(true);
  }, []);

  const { setProjectModalToggle } = useStateStore();
  const { updateSpecificProject } = useProjectStore();
  ...
  const onFinish = (values: FormData) => {
    ...
    setTimeout(() => {
      setLoading(false);
      const { email, ...allData } = values;
      const updatedProject = { ...allData, ...teamMembers };
      // Show success message
      updateSpecificProject(updatedProject);
    ...
    <div style={{ padding: "20px" }}>
      <Spin spinning={loading} tip="Submitting..." size="large">
        <Form form={form} layout="vertical" onFinish={onFinish}>
          ...
          >
            <TextArea defaultValue={project.details} rows={4} />
          </Form.Item>
          <Form.Item>
